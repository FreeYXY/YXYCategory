...
//    return [NSString stringWithFormat:@"%ld",(long)delta.second] ;
//}
...
//传入今天的时间，返回下个月
+ (NSString *)getNextMonth:(NSDate *)aDate {
    NSCalendar *gregorian = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    gregorian.locale = [[NSLocale alloc] initWithLocaleIdentifier:@"zh_CN"];
    NSDateComponents *components = [gregorian components:NSCalendarUnitWeekday | NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay fromDate:aDate];
//    [components setDay:([components day]+1)];
...
- (NSString *)friendlyDateString{
    ...
    if ([self year]==[[NSDate date] year]) {
        NSInteger days = [NSDate daysOffsetBetweenStartDate:self endDate:[NSDate date]];
        if (days <= 2) {
            dateStr = [self stringYearMonthDayCompareToday];
        }else{
            dateStr = [self stringMonthDay];
        }
    }else{
        dateStr = [self stringYearMonthDay];
    }
    
    
    if ([self hour]>=5 && [self hour]<12) {
        period = @"上午";
        hour = [NSString stringWithFormat:@"%02d",(int)[self hour]];
    }else if ([self hour]>=12 && [self hour]<=18){
        period = @"下午";
        hour = [NSString stringWithFormat:@"%02d",(int)[self hour]-12];
    }else if ([self hour]>18 && [self hour]<=23){
        period = @"晚上";
        hour = [NSString stringWithFormat:@"%02d",(int)[self hour]-12];
    }else{
        period = @"早上";
    ...
    return [NSString stringWithFormat:@"%@ %@ %@:%02d",dateStr,period,hour,(int)[self minute]];
}
...
- (NSDate *)beginningOfWeek {
    ...
    NSUInteger offset = ([components weekday] == [calendar firstWeekday]) ? -1 : [components weekday] - 2;
...
- (BOOL)isBigThan:(NSDate*)date
{
    return  [self timeIntervalSinceDate:date] > 0;
}

- (BOOL)isEqWidth:(NSDate*)date
{
    return  [self timeIntervalSinceDate:date] == 0;
}
@end
