...
static NSString *const ASSOCIATION_MUTABLE_CCNAME = @"ASSOCIATION_MUTABLE_CCNAME";
static NSString *const ASSOCIATION_MUTABLE_CCSHOW = @"ASSOCIATION_MUTABLE_CCSHOW";
...
- (void)setCcName:(NSString *)ccName {
    objc_setAssociatedObject(self,(__bridge const void *)(ASSOCIATION_MUTABLE_CCNAME),ccName,OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSString *)ccName {
    return objc_getAssociatedObject(self,(__bridge const void *)(ASSOCIATION_MUTABLE_CCNAME));
}

- (void)setCcIsShow:(BOOL)ccIsShow {
    objc_setAssociatedObject(self,(__bridge const void *)(ASSOCIATION_MUTABLE_CCSHOW),[NSNumber numberWithBool:ccIsShow],OBJC_ASSOCIATION_ASSIGN);
}

#pragma mark - 仅仅用来作为常用参数使用 无其他功能
- (BOOL)ccIsShow {
    NSNumber *number = [NSNumber numberWithBool:objc_getAssociatedObject(self, (__bridge const void *)(ASSOCIATION_MUTABLE_CCSHOW))];
...
//颜色渐变
- (void)mask:(NSArray *)colors Locations:(NSArray *)locations cornerRadius:(CGFloat)cornerRadius {
...
- (UIView *) createShadowViewWithRadius:(float)radius Color:(UIColor *)color Directions:(NSArray *)directions {
    ...
    for (NSString *direction in directions) [directionDict setValue:@"1" forKey:direction];
